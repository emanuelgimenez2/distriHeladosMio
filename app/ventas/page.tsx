...
export default function VentasPage() {
  ...
  const [selectedSale, setSelectedSale] = useState<SaleWithMixedDetails | null>(null)

  const [emitModalOpen, setEmitModalOpen] = useState(false)
  const [saleToEmit, setSaleToEmit] = useState<SaleWithMixedDetails | null>(null)
  ...
  const loadData = async () => {
    try {
      ...
      const enhancedSales = salesData.map((sale: Sale, index: number) => ({
        ...
        cashAmount: sale.paymentType === 'mixed' ? Math.floor(sale.total * 0.6) : undefined,
        creditAmount: sale.paymentType === 'mixed' ? Math.floor(sale.total * 0.4) : undefined,
      }))
    ...
    try {
      if (documentType === 'invoice') {
        ...
        try {
          ...
          clientData = clients.find((c: Client) => c.name === saleToEmit.clientName) || null
        } catch (e) {
          console.warn('No se pudo obtener datos del cliente:', e)
        }

        // Datos mínimos requeridos para la factura
        const invoicePayload = {
          name: saleToEmit.clientName || 'Consumidor Final',
          phone: saleToEmit.clientPhone || '',
          email: clientData?.email || '',
          taxCategory: clientData?.taxCategory || 'consumidor_final', // ← REQUERIDO - CON VALOR POR DEFECTO
        ...
        const result = await salesApi.emitInvoice(saleToEmit.id, invoicePayload)
      ...
      {loading ? (
        ...
        <div className="space-y-4 md:space-y-6 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-foreground tracking-tight">Historial de Ventas</h1>
              <p className="text-muted-foreground text-sm mt-1">
                {filteredSales.length} {filteredSales.length === 1 ? 'venta registrada' : 'ventas registradas'} 
                {periodFilter === 'today' && ' hoy'}
              </p>
            </div>
            <Button asChild className="w-full sm:w-auto gap-2 shadow-lg shadow-primary/20">
          ...
          <Card className="border-border/60 shadow-sm">
            <CardContent className="p-3 md:p-4">
              ...
              {/* Active Filters */}
              {(searchQuery || paymentFilter !== 'all' || invoiceFilter !== 'all' || periodFilter !== 'today') && (
                <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-border/50">
                  ...
                  {periodFilter !== 'today' && (
                    <Badge variant="secondary" className="text-xs gap-1">
                      ...
                      <button onClick={() => setPeriodFilter('today')}><X className="h-3 w-3" /></button>
                    </Badge>
                  )}
                  {paymentFilter !== 'all' && (
                    <Badge variant="secondary" className="text-xs gap-1">
                      {payLabel(paymentFilter)}
                      <button onClick={() => setPaymentFilter('all')}><X className="h-3 w-3" /></button>
                    </Badge>
                  )}
                  {invoiceFilter !== 'all' && (
                    <Badge variant="secondary" className="text-xs gap-1">
                      {invoiceFilter === 'emitted' ? 'Emitidas' : 'Pendientes'}
                      <button onClick={() => setInvoiceFilter('all')}><X className="h-3 w-3" /></button>
                    </Badge>
                  )}
                  <button 
                    onClick={() => {setSearchQuery(''); setPaymentFilter('all'); setInvoiceFilter('all'); setPeriodFilter('today')}}
          ...
          <Card className="border-border/60 shadow-sm overflow-hidden">
            {filteredSales.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-foreground mb-1">No se encontraron ventas</h3>
                  ...
                  >
                    ...
                    <div className="flex md:hidden items-start justify-between w-full">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-foreground">Venta N° {sale.saleNumber || filteredSales.length - index}</p>
                          <p className="text-xs text-muted-foreground">{sale.clientName || 'Venta directa'}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-foreground">{fmt(sale.total)}</p>
                        <Badge variant="outline" className={`text-[10px] mt-1 ${payBadgeCls(sale.paymentType)}`}>
                    ...
                    <div className="hidden md:flex md:col-span-2 items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-foreground text-sm">N° {sale.saleNumber || filteredSales.length - index}</p>
                        <p className="text-[10px] text-muted-foreground font-mono">{sale.id.slice(-6)}</p>
                      </div>
                    </div>
                    ...
                    <div className="hidden md:flex md:col-span-3 items-center gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-foreground truncate">{sale.clientName || 'Venta directa'}</p>
                        {sale.sellerName && (
                          <p className="text-[10px] text-muted-foreground truncate">Vendedor: {sale.sellerName}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="hidden md:flex md:col-span-2 items-center justify-end">
                      <p className="font-bold text-foreground text-base">{fmt(sale.total)}</p>
                    </div>

                    <div className="hidden md:flex md:col-span-2 items-center justify-center">
                      <Badge variant="outline" className={`gap-1.5 px-2.5 py-1 ${payBadgeCls(sale.paymentType)}`}>
                    ...
                    <div className="hidden md:flex md:col-span-1 items-center justify-center">
                      {sale.invoiceEmitted ? (
                        <div className="h-8 w-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center" title="Boleta emitida">
            ...
            <>
              ...
              <div className="p-6 space-y-6">
                    ...
                    <div className="flex items-center gap-2">
                      <Avatar className="h-8 w-8">
                        <AvatarFallback className="bg-primary/10 text-primary text-xs">
                          {(selectedSale.clientName || 'C').charAt(0).toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                      <p className="font-medium text-foreground">{selectedSale.clientName || 'Cliente final'}</p>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Vendedor</p>
                    <p className={`font-medium ${selectedSale.sellerName ? 'text-foreground' : 'text-muted-foreground italic'}`}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className={`p-3 rounded-xl border ${selectedSale.invoiceEmitted ? 'bg-emerald-50/50 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800' : 'bg-amber-50/50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800'}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <FileText className={`h-4 w-4 ${selectedSale.invoiceEmitted ? 'text-emerald-600' : 'text-amber-600'}`} />
                      <span className="text-xs font-medium text-muted-foreground">Boleta</span>
                    </div>
                    <p className={`font-semibold ${selectedSale.invoiceEmitted ? 'text-emerald-700 dark:text-emerald-400' : 'text-amber-700 dark:text-amber-400'}`}>
                      {selectedSale.invoiceEmitted ? (selectedSale.invoiceNumber || 'Emitida') : 'Pendiente'}
                    </p>
                  </div>
                  <div className={`p-3 rounded-xl border ${selectedSale.remitoNumber ? 'bg-blue-50/50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800' : 'bg-muted/50 border-border'}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <Truck className={`h-4 w-4 ${selectedSale.remitoNumber ? 'text-blue-600' : 'text-muted-foreground'}`} />
                      <span className="text-xs font-medium text-muted-foreground">Remito</span>
                    </div>
                    <p className={`font-semibold ${selectedSale.remitoNumber ? 'text-blue-700 dark:text-blue-400' : 'text-muted-foreground'}`}>
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    {selectedSale.items.map((item, i) => (
                      <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-muted/40 hover:bg-muted/60 transition-colors">
                        ...
                        <p className="font-semibold text-foreground">{fmt(item.price * item.quantity)}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {selectedSale.paymentType === 'mixed' && (
                  <div className="p-4 rounded-xl bg-amber-50/50 border border-amber-200 dark:bg-amber-900/20 dark:border-amber-800 space-y-2">
                    <p className="text-xs font-medium text-amber-800 dark:text-amber-400 uppercase tracking-wider">Detalle de Pago Mixto</p>
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2 text-sm text-amber-700 dark:text-amber-300">
                        <Banknote className="h-4 w-4" />
                        Efectivo
                      </div>
                      <span className="font-semibold text-amber-800 dark:text-amber-400">{fmt(selectedSale.cashAmount || 0)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2 text-sm text-amber-700 dark:text-amber-300">
                      ...
                      <span className="font-semibold text-amber-800 dark:text-amber-400">{fmt(selectedSale.creditAmount || 0)}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-foreground text-background">
                  ...
                  <span className="text-2xl font-bold">{fmt(selectedSale.total)}</span>
                </div>
                    ...
                    {selectedSale.invoicePdfUrl && (
                      <>
                        ...
                        {buildWhatsappUrl(selectedSale.clientPhone, selectedSale.invoicePdfUrl, selectedSale.clientName) && (
                          <Button variant="outline" size="sm" asChild className="flex-1 gap-1.5 bg-green-50 text-green-700 border-green-200 hover:bg-green-100 hover:text-green-800">
                ...
                <div className="flex gap-2 pt-2">
                  {!selectedSale.invoiceEmitted && (
                    <Button 
                      ...
                      onClick={() => { setDetailModalOpen(false); handleOpenEmitModal(selectedSale) }}
                    >
                  ...
                  {!selectedSale.remitoNumber && (
                    <Button 
                      variant="outline" 
                      className="flex-1 gap-1.5"
                      onClick={() => { setDetailModalOpen(false); setSaleToEmit(selectedSale); setDocumentType('remito'); setEmitModalOpen(true) }}
                    >
                  ...
                  {(selectedSale.invoiceEmitted && selectedSale.remitoNumber) && (
                    <Button variant="outline" className="w-full" onClick={() => setDetailModalOpen(false)}>
                      Cerrar
                    </Button>
                  )}
                </div>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-lg">
              <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center">
                {documentType === 'invoice' ? <FileText className="h-5 w-5 text-primary" /> : <Truck className="h-5 w-5 text-primary" />}
              </div>
            ...
            <div className="space-y-4 pt-2">
              <div className="p-4 rounded-xl bg-muted/50 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">N° {saleToEmit.saleNumber}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Cliente</span>
                  <span className="font-medium">{saleToEmit.clientName || 'Cliente final'}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Total</span>
                  <span className="font-bold text-lg">{fmt(saleToEmit.total)}</span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <FileText className={`h-6 w-6 mb-2 ${documentType === 'invoice' ? 'text-primary' : 'text-muted-foreground'}`} />
                    <p className={`font-semibold ${documentType === 'invoice' ? 'text-foreground' : 'text-muted-foreground'}`}>Boleta</p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {saleToEmit.invoiceEmitted ? 'Ya emitida' : 'Factura fiscal digital'}
                    </p>
                  </button>
                  <button
                  ...
                  >
                    <Truck className={`h-6 w-6 mb-2 ${documentType === 'remito' ? 'text-primary' : 'text-muted-foreground'}`} />
                    <p className={`font-semibold ${documentType === 'remito' ? 'text-foreground' : 'text-muted-foreground'}`}>Remito</p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {saleToEmit.remitoNumber ? 'Ya generado' : 'Comprobante de entrega'}
                    </p>
                  </button>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <Button variant="outline" className="flex-1" onClick={() => setEmitModalOpen(false)}>
                  Cancelar
                </Button>
                <Button 
                  className="flex-1 gap-2" 
                  onClick={handleEmitDocument} 
                  disabled={emitting || (documentType === 'invoice' && saleToEmit.invoiceEmitted) || (documentType === 'remito' && saleToEmit.remitoNumber)}
                >
              ...
              {lastInvoice?.afipData && (
                ...
                >
                  <BoletaDocument
                    ...
                    date={new Date()}
                    clientName={selectedClientData?.name || saleToEmit?.clientName}
                    clientCuit={selectedClientData?.cuit}
                    clientAddress={selectedClientData?.address}
                    clientPhone={saleToEmit?.clientPhone || selectedClientData?.phone}
                    clientTaxCategory={selectedClientData?.taxCategory}
                    items={saleToEmit?.items.map((item: any) => ({
                      name: item.name,
