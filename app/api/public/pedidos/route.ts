...
export async function POST(request: Request) {
  ...
  const existingClientSnap = await adminFirestore
    .collection("clientes")
    .where("dni", "==", dni)
    .limit(1)
    .get();
  if (!existingClientSnap.empty) {
    ...
    const clientRef = await adminFirestore.collection("clientes").add({
      name,
      dni,
      cuit: dni,
      email,
  ...
  if (sellerEmail) {
    const sellerSnap = await adminFirestore
  ...
  const orderRef = await adminFirestore.collection("pedidos").add({
    saleId: null,
    clientId,
    clientName,
    sellerId,
