...
export function StoreFront({
  ...
  const [publicTaxCategory, setPublicTaxCategory] = useState<
    ...
    const loadSellerMatch = async () => {
      ...
      try {
        const response = await fetch(
          `/api/public/vendedores?email=${encodeURIComponent(user.email)}`,
        );
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      try {
        ...
        const response = await fetch(
          `/api/public/clientes?dni=${encodeURIComponent(publicDni.trim())}`,
        );
        const data = await response.json();
        if (data.found) {
          ...
          setPublicTaxCategory(
            (data.client.taxCategory ||
              "consumidor_final") as typeof publicTaxCategory,
          );
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10">
        {/* Header */}
        {showHeader && (
          <header className="sticky top-0 z-50 border-b border-border/60 bg-card/85 backdrop-blur supports-[backdrop-filter]:bg-card/75 shadow-sm">
        ...
        <main
          className={`container mx-auto px-4 ${showHeader ? "py-8" : "py-6 sm:py-8"}`}
        >
                    ...
                    <div className="flex flex-col gap-2">
                      {[
                        ...
                        <Button
                          ...
                          size="sm"
                          variant={
                            baseFilter === item.id ? "default" : "outline"
                          }
                          className="justify-start rounded-full"
                ...
                return (
                  ...
                  >
                    ...
                    <CardContent className="p-1.5 sm:p-3">
                      <Badge
                        variant="secondary"
                        className="mb-1 text-[7px] uppercase tracking-wide"
                      >
                        {product.category}
                      </Badge>
              ...
              {isPublicStore ? (
                <div className="space-y-3">
                  {sellerMatchName && (
                    <div className="rounded-lg border border-success/30 bg-success/10 px-3 py-2 text-sm text-success">
                      Pedido registrado a vendedor:{" "}
                      <span className="font-medium">{sellerMatchName}</span>
                    </div>
                  )}
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setPublicTaxCategory(
                          e.target.value as typeof publicTaxCategory,
                        )
                      }
                    >
                ...
                <>
                  <RadioGroup
                    value={paymentType}
                    onValueChange={(v) =>
                      setPaymentType(v as "cash" | "credit")
                    }
                    className="space-y-3"
                  >
                    ...
                    <div className="flex items-center space-x-3 p-4 rounded-lg border border-border hover:bg-muted/50 cursor-pointer">
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Se suma a la deuda del cliente
                          </p>
                        </div>
                      </Label>
                    </div>
